...
const Sidebar = () => {
  ...
  const month = today.toLocaleString("default", { month: "short" }); // Short month name (e.g., "Dec")
  const year = today.getFullYear();

  let hours = today.getHours();
  const minutes = today.getMinutes().toString().padStart(2, "0"); // Ensure two digits for minutes
  ...
  return (
    <div>
      ...
      <div
        className={`w-[310px] fixed bg-[#202123] z-50 top-0 h-screen shadow-[0_0_15px_0_rgb(34_41_47_/_10%)] transition-all`}
      >
        <div className="h-[70px] flex justify-center items-center">
          <Link to="/" className="w-[180px] h-[50px] ml-[-92px]">
            <img
              className="w-full"
                ...
                <Link
                  ...
                  className={`${
                    pathname === n.path
                      ...
                      : "text-gray-500 font-bold duration-200"
                  } px-[32px] py-[2px] flex justify-start items-center gap-[12px] hover:pl-[36px] transition-all w-full mb-6 text-xl`}
                >
                  <span>{n.icon}</span>
                  <span>{n.title}</span>
                </Link>
